...
impl<'a> DnsSocket<'a> {
    ...
    pub(crate) fn process(
        ...
        if p.opcode() != Opcode::Query {
            net_trace!("dns: unwanted opcode {:?}", p.opcode());
            return Err(Error::Malformed);
        }

        if !p.flags().contains(Flags::RESPONSE) {
            net_trace!("dns: packet doesn't have response bit set");
            return Err(Error::Malformed);
        }

        if p.question_count() != 1 {
            net_trace!("dns: bad question count {:?}", p.question_count());
            return Err(Error::Malformed);
        }
        ...
        for q in self.queries.iter_mut().flatten() {
            if let State::Pending(pq) = &mut q.state {
                ...
                if question.type_ != pq.type_ {
                    net_trace!("dns: question type mismatch");
                    return Err(Error::Malformed);
                }
                if !eq_names(p.parse_name(question.name), p.parse_name(&pq.name))? {
                    net_trace!("dns: question name mismatch");
                    return Err(Error::Malformed);
                }
        ...
        net_trace!("dns: no query matched");
        Ok(())
    }
